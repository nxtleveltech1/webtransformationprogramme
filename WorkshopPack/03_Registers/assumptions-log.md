...
**Source:** SRC-001, SRC-002 (Day 1); SRC-008/009 (Day 2). Assumptions implied by discussion that require validation. Day 2 assumptions are ASM-012–ASM-016.
...
| ASM-012 | Current team capacity is sufficient for the Personal segment only (not all segments concurrently) | Resourcing/Scope | Programme | Confirm via four-squad resourcing (ACT-064) | Year-end slip if more segments expected concurrently | SRC-009 |
| ASM-013 | A two-person squad completes a macro section per sprint; full Personal section ≈ 5 sprints | Delivery/Estimation | Design/Programme | Validate against actual sprint throughput | Roadmap dates invalid | SRC-008/009 |
| ASM-014 | ~2h/page build holds for the ~850 remaining pages once templates/components are ready | Publishing/Capacity | Publishing | Validate vs real throughput | Capacity model & timeline invalid | SRC-008 |
| ASM-015 | ~60% of modules are known and can start now; the remainder firmed section-by-section | Delivery/Scope | Design/Programme | Validate as sections complete | Re-planning churn | SRC-008 |
| ASM-016 | Template/component parallel development is feasible (component library built without full templates) | Technical/Delivery | Design System | Validate DS build process | Delivery sequencing risk | SRC-008 |

---
> Day 2 assumptions added as ASM-012–ASM-016.
